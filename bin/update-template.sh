#!/usr/bin/env bash
# updates a repository built from the base template, with the latest changes.
...
[[ $(git remote show template 2>/dev/null) ]] || {
  git remote add template "https://github.com/jmpa-oss/docker-actions-template.git" \
